...
class UniversalModelLauncherUI(QMainWindow):
    ...
    def _connect_signals(self):
        ...
        # Main panel model loading requests
        self.main_panel.model_launch_requested.connect(self._handle_model_launch)
        
        # Connect model discovery to left panel
        self.main_panel.models_discovered.connect(self.left_panel.populate_models)
        
        # Left panel to main panel - MODEL TYPE FILTERING
        self.left_panel.model_type_filter_changed.connect(self.main_panel.set_type_filter)
    ...
    def _handle_model_launch(self, config: dict):
        ...
        # Execute in background using QTimer for GUI thread safety
        def start_loading():
            try:
                import asyncio
                loop = asyncio.new_event_loop()
                ...
                self.right_panel.add_log_entry(f"❌ Model loading failed: {e}", "error")
    ...
    def closeEvent(self, event):
        ...
        # Optional: Add fade out animation that doesn't block closing
        if hasattr(self, 'animator'):
            self.animator.fade_out_widget(self, duration=200)
