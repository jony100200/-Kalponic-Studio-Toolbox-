...
class HardwareDetector:
    ...
    def get_system_info(self) -> Dict:
        """Get system information in GUI-compatible format"""
        import psutil
        memory = psutil.virtual_memory()
        
        result = {
            'platform': self.system_info.platform if self.system_info else 'Unknown',
            'ram_total_gb': self.system_info.total_ram_gb if self.system_info else 0,
        ...
        if self.gpus:
            primary_gpu = self.gpus[0]  # Use first GPU
            result.update({
                'gpu_name': primary_gpu.name,
                'gpu_memory_gb': primary_gpu.memory_gb,
                'cuda_available': primary_gpu.vendor == 'nvidia' and primary_gpu.is_available,
                'cuda_version': primary_gpu.driver_version or 'N/A'
            })
