...
class TestBackendRegistry:
    ...
    def test_unknown_backend_error(self):
        """Test that unknown backends raise appropriate errors."""
        with pytest.raises(ValueError, match="Unknown segmenter backend"):
            segment.get_segmenter_backend('nonexistent')

        with pytest.raises(ValueError, match="Unknown matte backend"):
            matte.get_matte_backend('nonexistent')

        with pytest.raises(ValueError, match="Unknown part backend"):
            parts.get_part_backend('nonexistent')