...
class _DummyConfig:
    ...
    def __init__(self, bridge_rel_path: str):
        ...
        self.targets = {"copilot": {}, "kilo": {}}
        self.enabled_targets = ["copilot", "kilo"]
...
class TestJobRunnerKiloAdapter(unittest.TestCase):
    def test_kilo_vscode_adapter_uses_bridge_capture_and_contract_artifacts(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            plan = {
                "name": "kilo_adapter_test",
                "steps": [
                    {
                        "id": "kilo_worker",
                        "type": "worker_contract",
                        "target": "kilo",
                        "content": "Summarize implementation blockers",
                        "worker": {"adapter": "kilo_vscode"},
                        "output_file": "outputs/kilo_worker.md",
                        "validator": {"type": "sections", "required": ["# Kilo Output"]},
                    }
                ],
            }
            ...
            self.assertEqual(call["target_name"], "kilo")
            self.assertTrue(call["allow_command_open"])
            self.assertTrue(call["press_enter"])

            output_path = os.path.join(temp_dir, "outputs", "kilo_worker.md")
            self.assertTrue(os.path.exists(output_path))
            with open(output_path, "r", encoding="utf-8") as handle:
                output = handle.read()
            self.assertIn("# Kilo Output", output)

            worker_dir = os.path.join(temp_dir, "artifacts", "kilo_worker", "attempt_1_worker_contract")
            ...
            self.assertEqual(response.get("target"), "kilo")
