...
class TestJobRunnerGeminiAdapter(unittest.TestCase):
    def test_gemini_vscode_adapter_uses_bridge_capture_and_contract_artifacts(self):
        with tempfile.TemporaryDirectory() as temp_dir:
                ...
                "steps": [
                    {
                        ...
                        "worker": {"adapter": "gemini_vscode"},
                        "output_file": "outputs/gemini_worker.md",
                        "validator": {"type": "sections", "required": ["# Gemini Output"]},
                    }
                ],
            }
            ...
            self.assertEqual(call["target_name"], "gemini")
            self.assertTrue(call["allow_command_open"])
            self.assertTrue(call["press_enter"])

            output_path = os.path.join(temp_dir, "outputs", "gemini_worker.md")
            self.assertTrue(os.path.exists(output_path))
            with open(output_path, "r", encoding="utf-8") as handle:
                output = handle.read()
            self.assertIn("# Gemini Output", output)

            worker_dir = os.path.join(temp_dir, "artifacts", "gemini_worker", "attempt_1_worker_contract")
            ...
            self.assertEqual(response.get("target"), "gemini")
